// TO THE REVIEWER READING THIS, I USED THE FORMAT DOCUMENT FUNCTION IN VISUAL STUDIO.
// SORRY IF THIS IS HELL FOR YOU.
...
void UIController::setupCustomDrawGameState() {
...
#elif defined __linux__ || defined __ORBIS__ || defined __PS3__ || \
    defined __PSVITA__
    RenderManager.StartFrame();
#endif
    ...
    // orthographic covering the Iggy screen space
    RenderManager.MatrixMode(GL_PROJECTION);
    RenderManager.MatrixSetIdentity();
    RenderManager.MatrixOrthogonal(0.f, m_fScreenWidth, m_fScreenHeight, 0.f,
                                   1000.f, 3000.f);
...
void UIController::setupCustomDrawMatrices(UIScene* scene,
    ...
    // Compute the pixel-space bounding rect for this control (used for the
    // depth-buffer clear at the end of the custom draw pass)
    ...
    float tx =
        ...
        2.f;
    float ty = (m_fScreenHeight -
                customDrawRegion->mat[(1 * 4) + 3] * m_fScreenHeight) /
               2.f;
    RenderManager.MatrixTranslate(tx, ty, 0.f);

    float sx = (m_fScreenWidth * customDrawRegion->mat[0]) / 2.f;
    float sy = (m_fScreenHeight * -customDrawRegion->mat[(1 * 4) + 1]) / 2.f;
    RenderManager.MatrixScale(sx, sy, 1.0f);
}
